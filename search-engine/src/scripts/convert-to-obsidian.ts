...
function main(): void {
  ...
  const data = JSON.parse(raw) as RawGraphOutput

  fs.mkdirSync(outputDir, { recursive: true })

  // 1. Créer un fichier par entité
  data.merged_entities?.forEach((entity: RawEntity) => {
    const content =
      `# ${entity.name}\n\n` +
      `**Type**: ${entity.type}\n` +
  ...
  data.chapters.forEach((chapter: ChapterGraph) => {
    chapter.relations.forEach((rel: RawRelation) => {
